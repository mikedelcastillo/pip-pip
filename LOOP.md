...
| 158| (latest)    | PERF (client FPS): cache-gate the per-player health-bar overlay so it only redraws (clear()+lineStyle/geometry) when its state CHANGES (health/maxHealth/bar-colour) instead of every frame for every player; pure healthOverlayChanged helper unit-tested. Animated buff rings moved to a separate per-frame buffGraphic so they are NOT frozen; buffPulse clock hoisted to once-per-frame. Frame-for-frame identical output | `git revert <sha>` |
