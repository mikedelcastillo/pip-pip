...
| 134| (latest)    | FIX bots cant navigate complex 1-tile corridors/mazes: nav grid now TILE-ALIGNED (one cell per map tile, centred on tiles) + clearance = ship+wall radius + raised cell cap, so corridor tiles are open cells (maze now REACHABLE end to end) | `git revert <sha>` |
