...
| 139| (latest)    | Editor tooltip no longer flashes on touch: onFocus is gated to keyboard focus (:focus-visible) so a tap's residual button focus does not pop the bubble; hover path was already touch-gated. Mobile-UX polish on the portal tooltip from #137 | `git revert <sha>` |
