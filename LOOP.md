...
| 182| `de81fd3`   | BUGFIX (medium, found by bug-hunt #5 - client tier): renderer LEAKED a PlayerGraphic on every player/bot removal. The removePlayer handler did delete + playersContainer.removeChild but never destroyed the graphic; PlayerGraphic had NO destroy() and was never destroyed anywhere. removeChild only DETACHES, so each removed player orphaned its name PIXI.Text (a canvas-backed GPU texture) + two PIXI.Graphics geometry buffers, unreclaimable for the session (and the teardown app.destroy passes texture:false, so it never freed the Text textures either). Bot churn (add/clear/fill) fires removePlayer repeatedly -> a GPU/memory leak that can blank the canvas / OOM a phone (mobile is a priority). FIX: added PlayerGraphic.destroy() -- detaches + destroys nameText WITH its texture, then container.destroy({children:true}) frees the Graphics + sprite display objects while the default texture:false PRESERVES the SHARED ship texture -- called from the removePlayer handler AND for every player still present in renderer.destroy(). CLIENT-ONLY. client tsc(4.8.4) + vite build + full suite + client lint green | `git revert de81fd3` |
| 183| `cb67db8`   | BUGFIX (medium, found by bug-hunt #5 - client tier): HELD key/mouse state stayed STUCK after Alt+Tab. KeyboardListener/MouseListener bind keyup/mouseup on WINDOW, so a release while the window is blurred (Alt+Tab / Cmd+Tab / OS focus theft) is delivered to the OTHER app and never seen -> the held key/button stays down. The update tick is a setInterval Ticker that keeps firing while blurred, so processInputs reads the stuck state and sendPackets transmits movementAmount=1 / useWeapon=true every tick -> the ship keeps FLYING/FIRING in the authoritative sim while the player is in another app, stopping only if they press+release that exact key again after refocusing. Both event maps already DECLARED blur/focus but nothing emitted/consumed them (dead intent). FIX: KeyboardListener clears state={} and MouseListener zeroes left/middle/right down+dragging on a window blur (and document visibilitychange when hidden), emitting the declared blur event; handlers bound-once and removed in destroy(); document guarded with typeof for node/SSR safety (keeps the existing listener-symmetry test green). CORE-only. Unit-tested (blur clears held keys/buttons, emits blur, stops after destroy); core tsc + client tsc(4.8.4) + core lint + full suite green | `git revert cb67db8` |
| 184| `9932786`   | BUGFIX (medium, found by bug-hunt #5 - client tier): reconnect() did not re-sync game.clientPlayerId. It is set ONCE in mountGameView to the connection id and updated nowhere else. If the socket drops and the server PRUNES the connection during the outage (~10-min idle lifespan), Reconnect re-verifies, fails, and requestConnection assigns a BRAND-NEW connectionId; the server re-adds the local player under the new id, but the cached game.clientPlayerId stays the OLD (now-nonexistent) id. Consequence: playerIsClient = (player.id === clientPlayerId) is FALSE for the real local player, so client-side bullet PREDICTION is disabled (bullets only appear on the next server broadcast, adding latency), and the camera-shake, minimap local marker + kill-feed highlight all key off a player id that no longer exists. FIX: re-assign this.game.clientPlayerId = this.client.connectionId after connect+joinLobby in reconnect(), exactly as mountGameView does. CLIENT-ONLY. (No dedicated test: GameContext is a network singleton; the fix is a verbatim mirror of the verified mountGameView assignment, gated by client tsc 4.8.4 + full suite + client lint.) | `git revert 9932786` |
| 185| (latest)    | BUGFIX (low, found by bug-hunt #5, completes 5/5): kill/powerup FEED entry identity. Two related defects in the store feed writers: (1) ZOMBIE buff row -- addPowerupPickup appended without dedup and entries are never pruned on buff expiry, so re-acquiring (or refreshing) a timed buff while an old same-player+type entry was still in the 6-slot feed made visibleTacticalPowerups (which keys live remaining ticks on player+type) annotate BOTH with the one live value -> two identical 'AA HASTE' countdown rows, or a long-expired row resurrected. (2) DUPLICATE React keys -- the entry id was Date.now()+feed.length, which degenerates at the cap (slice pins length, so a same-ms multi-kill / multi-pickup batch all got the same key). FIX: (a) a module-level MONOTONIC counter for the id (always unique; kill + powerup feeds); (b) addPowerupPickup drops any prior same player+type entry for a TIMED buff before appending (instant health/ammo unchanged). CLIENT-ONLY. Unit-tested (timed re-pickup -> 1 row, distinct players/types kept, unique ids at the cap); client tsc(4.8.4) + full suite + client lint green. (A 6th hunt-#5 finding -- the same key collision -- was REJECTED by an independent verifier as no real user impact, dev-only React warning; fixed anyway as cheap correctness hygiene.) | `git revert <sha>` |
