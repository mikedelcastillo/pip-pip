...
| 160| (latest)    | Lobby SAVED-MAP picker: completes make->save->play. Host-only "Saved maps" section in MapSelect lists the editor's localStorage library (read-only); tapping a row loads + validates it (loadMapFromLibrary) and applies via the same GAME_CONTEXT.setCustomMap path as upload/editor-stash; corrupt entry shows an inline error and sends nothing. Empty library renders nothing; 44px scrollable rows at 393px; key-isolated read-only. CLIENT-ONLY | `git revert <sha>` |
