...
| 151| (latest)    | Map editor KEYBOARD SHORTCUTS + controls reference: a collapsed <details> in the Options panel listing every tool key (derived from SHORTCUT_FOR/LABEL_FOR so it stays in sync) + undo/redo/fit/options/save + pointer gestures (paint, two-finger pan/pinch, alt-click eyedropper, the Half/Auto-slope flyouts). No always-visible chrome; Playwright-verified at 393px | `git revert <sha>` |
