...
| 152| (latest)    | Map editor MIRROR action (build symmetric arenas in one click, Aseprite-like): Options buttons "Mirror left/right" + "Mirror top/bottom" reflect everything painted across the bbox centre, flipping each shape (pure mirrorShape: diag/half flips per axis, full/deco unchanged) and reflecting spawns, via setCell/toggleSpawn so palette + spawn/tile exclusion hold; one undo step; centre-line cells map to themselves. Pure + unit-tested (involution + reflections); Playwright-verified at 393px | `git revert <sha>` |
