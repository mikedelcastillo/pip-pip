...
| 137| (latest)    | Editor polish: UNBOUNDED canvas (sparse Map model, no size inputs; bbox computed at export so the map is as large as you paint), portal Tooltip (createPortal to body so it escapes the tool-rail overflow clip), spawn/block mutual exclusion both ways | `git revert <sha>` |
