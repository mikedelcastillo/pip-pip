...
| 155| (latest)    | MOBILE FIX: paint applies on RELEASE, not press; pinch no longer drops a tile/spawn. Freehand/fill tap was applied on pointer-DOWN, so the first finger of a pinch painted before the 2nd landed (and the 2nd-finger handler then COMMITTED it). Now a tap paints on pointer-UP, a freehand drag only starts once the pointer leaves its start cell, and a 2nd finger CANCELS a gesture that has not drawn yet (pinch never paints). Playwright-verified: pinch=no paint, tap=one cell on release, drag=continuous stroke | `git revert <sha>` |
