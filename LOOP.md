...
| 133| (latest)    | FIX bots wiggling in place instead of chasing: stuck-detector only fires when actually TRAVELING (not orbit/retreat), measures progress over the full window, and clamps the progress unit so coarse grids dont flag a moving bot | `git revert <sha>` |
