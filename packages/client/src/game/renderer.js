...
export class GraphicPool {
    stage;
    graphics = [];
    Graphic;
    constructor(stage, Graphic) {
        this.stage = stage;
    ...
    free(graphic) {
        if (!(this.graphics.includes(graphic)))
            return;
...
export class PipPipRenderer {
    ...
    constructor(game) {
        ...
        this.buldgePinchFilter.center = new Point(0.5, 0.5);
        ...
        for (let i = 0; i < STAR_BG.COUNT; i++) {
            ...
            if (typeof starTexture === "undefined")
                continue;
    ...
    render(gameContext, deltaMs) {
        ...
        for (const graphic of this.bullets.active) {
            ...
            if (typeof graphic.bullet !== "undefined") {
                ...
                graphic.setPosition(graphic.positionX + (tx - graphic.positionX) * bulletSmoothing, graphic.positionY + (ty - graphic.positionY) * bulletSmoothing);
            }
            else {
            ...
            for (let i = 1; i < graphic.positions.length; i++) {
                ...
                graphic.graphic.moveTo(prev.x + prev.dx * (1 - PT), prev.y + prev.dy * (1 - PT));
                graphic.graphic.lineTo(cur.x + cur.dx * (1 - CT), cur.y + cur.dy * (1 - CT));
            }
