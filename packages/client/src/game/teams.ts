import { PipPipGameMode, MAX_TEAMS, teamIndices } from "@pip-pip/game/src/logic"
...
// The palette must provide one color per team the game logic supports; derive the
// cap from MAX_TEAMS so the palette and the authoritative team count can't drift.
export const MAX_TEAM_COLORS = MAX_TEAMS
...
export function teamColor(team: number): string {
    if (isRealTeam(team)) return TEAM_COLORS[team] ?? "#FFFFFF"
    return "#FFFFFF"
}
...
// The team indices for a given team count, re-exported from the game logic so the
// HUD leaderboard + scoreboard group by the exact same authoritative team set the
// server uses. No client-side reimplementation that could drift on the bounds.
export { teamIndices }
