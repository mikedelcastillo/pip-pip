...
import { POWERUP_BLOCK_SIZE } from "@pip-pip/game/src/logic/powerup-config"
...
// every spawned powerup lands on an empty (value 0) tile and never on a
// solid (full) or deco tile. A 14x14 grid (196 cells) leaves plenty of empties
// (>= POWERUP_BLOCK_SIZE^2 = 64) so the density target is > 0 and spawning runs.
//
...
// and a deco tile. Both must be excluded from powerup spawns.
...
// target floor(emptyTiles / POWERUP_BLOCK_SIZE^2).
...
// Map a powerup's world position back to its grid cell (inverse of the spawn
...
function makeSpawnGame(){
    const game = new PipPipGame({ spawnPowerups: true })
    game.setCustomMap(knownMixMap())
    game.setPhase(PipPipGamePhase.MATCH)
    return game
}

describe("tile-based powerup spawning", () => {
    it("the fixture's density target equals floor(emptyTiles / BLOCK_SIZE^2)", () => {
        const game = makeSpawnGame()
        const empty = countEmptyTiles()
        const expected = Math.floor(empty / (POWERUP_BLOCK_SIZE * POWERUP_BLOCK_SIZE))
        expect(game.powerupDensityTarget()).toBe(expected)
        // Sanity: the fixture is big enough to spawn at least one powerup.
        expect(expected).toBeGreaterThan(0)
    })

    it("only ever places powerups on EMPTY tiles, never a solid or deco tile", () => {
        const game = makeSpawnGame()
        ...
        for(let i = 0; i < game.POWERUP_SPAWN_INTERVAL_TICKS * 30; i++) game.update()

        const active = game.powerups.getActive()
        expect(active.length).toBeGreaterThan(0)
        for(const powerup of active){
            const { col, row } = cellOf(powerup.position.x, powerup.position.y)
            // In range.
    ...
    it("never places two active powerups on the same tile cell", () => {
        const game = makeSpawnGame()
        for(let i = 0; i < game.POWERUP_SPAWN_INTERVAL_TICKS * 30; i++) game.update()
    ...
    it("converges to AT MOST the density target active powerups", () => {
        const game = makeSpawnGame()
        const target = game.powerupDensityTarget()
        for(let i = 0; i < game.POWERUP_SPAWN_INTERVAL_TICKS * 50; i++) game.update()
    ...
    it("adds at most one powerup per spawn interval", () => {
        const game = makeSpawnGame()
        const interval = game.POWERUP_SPAWN_INTERVAL_TICKS

        let previous = game.powerups.getActive().length
        ...
        for(let i = 0; i < interval * 6; i++){
            game.update()
            const current = game.powerups.getActive().length
            const added = current - previous
            expect(added).toBeLessThanOrEqual(1)
            expect(added).toBeGreaterThanOrEqual(0) // spawning never removes a powerup here
            previous = current
        }
    })

    it("adds no more than one powerup across each full interval window", () => {
        const game = makeSpawnGame()
        const interval = game.POWERUP_SPAWN_INTERVAL_TICKS

        // Across any single interval-length window the active count grows by at
        // most POWERUP_SPAWN_PER_INTERVAL (= 1), proving the cadence gate.
        for(let window = 0; window < 5; window++){
            const before = game.powerups.getActive().length
            for(let i = 0; i < interval; i++) game.update()
            const after = game.powerups.getActive().length
