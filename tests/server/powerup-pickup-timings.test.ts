...
// A gameEvents stub that surfaces exactly one powerupPickup event (no others).
// encode.powerupPickup only reads powerup.id, so a minimal stub powerup suffices.
function eventsWithPickup(powerup: unknown, player: unknown){
    return {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        filter: (name: string) => (name === "powerupPickup" ? [{ powerupPickup: { powerup, player } }] : []) as any,
    }
}
...
// the powerup-feed countdown until the picker next reloaded or respawned.
describe("powerup pickup broadcasts the picker's ship timings", () => {
    it("includes the picker's playerShipTimings packet in the broadcast", () => {
        ...
        // The buff applyPowerupEffect set on the picker at pickup time.
        picker.ship.timings.haste = 200

        const powerup = { id: "p1" }
        // Broadcast to a DIFFERENT recipient (BB): it must learn AA's new timings.
        const message = getPartialGameState(contextFor(game, "BB", eventsWithPickup(powerup, picker))).flat()
